...
9. [signal_ollama.py — Reply to Yourself with Ollama](#9-signal_ollamapy--reply-to-yourself-with-ollama)
...
The automated update script (see [LAUNCHD_AUTOREBUILD.md](LAUNCHD_AUTOREBUILD.md))
carefully preserves the volume during rebuilds.

---

## 9. signal_ollama.py — Reply to Yourself with Ollama

`signal_ollama.py` turns your Signal note-to-self conversation into a chat interface
...
iPhone (note-to-self)
      ...
      ▼
signal-cli daemon  ──SSE──►  signal_ollama.py
                                    │
                                    ▼
                              Ollama (gemma3:12b)
                                    │  "Paris."
...
```bash
# At startup
OLLAMA_MODEL=gemma3:4b python signal_ollama.py
...
Edit `SYSTEM_PROMPT` in `signal_ollama.py`:
...
<dict>
    ...
    <array>
        ...
        <string>/Users/YOUR_USERNAME/Code/signal-cli/signal_ollama.py</string>
    </array>
...
For automated rebuilds, see **[LAUNCHD_AUTOREBUILD.md](LAUNCHD_AUTOREBUILD.md)**.
